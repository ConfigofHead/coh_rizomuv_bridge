...
def delete_polyset_material_tags():
    ...
    sel_mesh = lx.evalN('query sceneservice selection ? mesh')
    num_polset = lx.eval('query layerservice polset.N ? selected')
...
def store_sel_meshes():
    ...
    if not sel_layers:
        ...
        lx.eval('dialog.msg {You need to have at least one mesh item selected.}')
        lx.eval('dialog.open')
        sys.exit()
    return sel_layers
...
def from_rizom():
    ...
    try:
        ...
        imported_sel = lx.evalN('query sceneservice selection ? mesh')
        mesh_names = []
        ...
        for mesh in mesh_names:
            for uvmap in uvmap_name:
                ...
                sel = lx.evalN('query sceneservice selection ? mesh')
                lx.eval('uv.copy')
